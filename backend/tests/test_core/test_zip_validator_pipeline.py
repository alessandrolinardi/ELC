...
# =========================================================================
# 5. _update_street2 tests
# =========================================================================

class TestUpdateStreet2:
    """Tests for _update_street2 method."""

    def _run_update(self, existing_street2, location_info_outcome="",
                    location_info_parsed=""):
        """Helper: build a DF, run _update_street2, return the updated value."""
        validator = _make_validator()
        df = pd.DataFrame([{
            "Street 1": "Via Roma 10", "City": "Roma", "Zip": "00100",
            "Street 2": existing_street2,
        }])
        col_map = validator._map_columns(df)
        outcome = _make_outcome(location_info=location_info_outcome)
        parsed = _make_parsed()
        parsed.location_info = location_info_parsed
        idx = 0
        row = df.iloc[0]
        validator._update_street2(df, idx, row, col_map, outcome, parsed)
        return str(df.at[0, "Street 2"])

    def test_location_info_written_to_empty_street2(self):
        result = self._run_update("", location_info_outcome="C.C. Le Grange")
        assert result == "C.C. Le Grange"

    def test_location_info_merged_with_existing(self):
        result = self._run_update("Apt 3", location_info_outcome="Scala B")
        assert "Apt 3" in result
        assert "Scala B" in result
        assert " - " in result

    def test_location_info_not_duplicated(self):
        """If location_info is already present in street2, don't duplicate it."""
        result = self._run_update("Scala B", location_info_outcome="Scala B")
        assert result == "Scala B"
        assert result.count("Scala B") == 1

    def test_trailing_dashes_cleaned_from_existing(self):
        """Existing street2 trailing dashes should be cleaned before merge."""
        result = self._run_update("Apt 3 --", location_info_outcome="")
        # No location_info, but trailing dashes should be cleaned
        assert result == "Apt 3"

    def test_nan_street2_treated_as_empty(self):
        result = self._run_update("nan", location_info_outcome="Piano 2")
        assert result == "Piano 2"

    def test_parsed_location_info_used_when_outcome_empty(self):
        """Parsed location_info should be used when outcome location_info is empty."""
        result = self._run_update("", location_info_outcome="",
                                  location_info_parsed="Int. 5")
        assert result == "Int. 5"

    def test_no_street2_col_does_nothing(self):
        """When there is no Street 2 column, _update_street2 should not fail."""
        validator = _make_validator()
        df = pd.DataFrame([{
            "Street 1": "Via Roma 10", "City": "Roma", "Zip": "00100",
        }])
        col_map = validator._map_columns(df)
        outcome = _make_outcome(location_info="something")
        parsed = _make_parsed()
        # Should not raise
        validator._update_street2(df, 0, df.iloc[0], col_map, outcome, parsed)
